...
PREMIUM_CONTENT_TYPES = {
    "deep_dive": ContentType(
        key="deep_dive",
        label="Глубокий разбор",
        weight=5,
        emoji="\U0001f9ec",
    ),
    "protocol_plus": ContentType(
        key="protocol_plus",
        label="Протокол+",
        weight=4,
        emoji="\U0001f48e",
    ),
    "weekly_digest": ContentType(
        key="weekly_digest",
...
PREMIUM_RUBRIC_TAGS = {
    "deep_dive": "\U0001f9ec ГЛУБОКИЙ РАЗБОР",
    "protocol_plus": "\U0001f48e ПРОТОКОЛ+",
    "weekly_digest": "\U0001f4f0 ДАЙДЖЕСТ",
}
...
PREMIUM_SYSTEM_PROMPT = """Ты — контент-редактор премиум Telegram-канала SpectrMind о нейронауке и нейропсихологии.

Это ЭКСКЛЮЗИВНЫЙ канал для подписчиков. Контент должен быть более глубоким и детальным.

Правила:
- Пиши ТОЛЬКО на русском языке
- Научный стиль с глубоким анализом, но доступный обычному человеку
- Ссылайся на реальные исследования (Huberman Lab, Stanford, MIT, Nature, PubMed, Science, Cell, Neuron)
- Приводи конкретные данные: проценты, цифры, сроки экспериментов
- НЕ давай медицинских рекомендаций — только научные факты и общие советы
- НЕ используй слова: пептиды, инъекции, биомаркеры, диагноз, лечение
- НЕ используй markdown-разметку: никаких звёздочек (*), подчёркиваний (_), обратных кавычек (`), решёток для заголовков (#) — только обычный текст и эмодзи
- Используй эмодзи умеренно (3-5 на пост)
- Пост должен быть 400-700 слов (значительно длиннее обычных постов)
- Структура: заголовок → контекст → детальный анализ → практические выводы → хештеги
- В конце: 4-6 хештегов из этого набора: #нейронаука #мозг #продуктивность #нейропсихология #фокус #сон #дофамин #нейропластичность #саморазвитие #энергия #исследования #наука
- Начинай пост с тега-рубрики (ГЛУБОКИЙ РАЗБОР / ПРОТОКОЛ+ / ДАЙДЖЕСТ)
- ВАЖНО: ТЕМА указана в начале сообщения. Пиши СТРОГО про неё. Не придумывай свою тему."""
...
PREMIUM_PROMPTS = {
    "deep_dive": """Напиши глубокий разбор исследования для премиум Telegram-канала SpectrMind.

Формат поста:
[Эмодзи] ГЛУБОКИЙ РАЗБОР: [Заголовок с интригой]

[Контекст: почему это исследование важно, какую проблему решает]
[Методология: подробно — кто участвовал, сколько человек, какая продолжительность, какие методы использовались]
[Ключевые результаты: 3-5 конкретных выводов с цифрами и процентами]
[Механизмы: как это работает на уровне нейробиологии]
[Практическое применение: 3-4 конкретных действия на основе результатов]
[Ограничения исследования: что авторы признали]
[Источник: полное название, авторы, журнал, год]

#хештеги""",

    "protocol_plus": """Напиши детальный протокол для премиум Telegram-канала SpectrMind.

Формат поста:
[Эмодзи] ПРОТОКОЛ+: [Название протокола]

[Проблема: какую проблему решает этот протокол, почему это важно]
[Научная база: 2-3 исследования, подтверждающих эффективность]
[Пошаговый протокол: 5-7 шагов с детальными пояснениями каждого]
[Дозировки/тайминг: конкретные цифры — когда, сколько, как долго]
[Ожидаемые результаты: что и когда почувствуешь]
[Противопоказания и ограничения: когда НЕ стоит делать]
[Советы для максимального эффекта: 2-3 лайфхака]

#хештеги""",

    "weekly_digest": """Напиши недельный дайджест открытий для премиум Telegram-канала SpectrMind.

Формат поста:
[Эмодзи] ДАЙДЖЕСТ НЕДЕЛИ: [Главная тема]

[Введение: почему эта неделя была важна для нейронауки]
...
[Главный вывод недели: что это значит для каждого из нас]
[Практический совет: одно действие на основе открытий недели]
...
def get_premium_content_type() -> str:
    """Pick a random premium content type using weighted selection."""
    types = list(PREMIUM_CONTENT_TYPES.values())
    weights = [t.weight for t in types]
    chosen = random.choices(types, weights=weights, k=1)[0]
    return chosen.key


def get_premium_prompt(content_type: str) -> str:
    """Get the generation prompt for a premium content type."""
    return PREMIUM_PROMPTS.get(content_type, PREMIUM_PROMPTS["deep_dive"])


def get_premium_rubric_tag(content_type: str) -> str:
    """Get the rubric tag for a premium content type."""
    return PREMIUM_RUBRIC_TAGS.get(content_type, "")
